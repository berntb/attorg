...
=pod
  ...
  Couple of technical notes:
     ...
     2. send_file was originally designed to send either physical
     files, or in-memory scalars.
        The streaming option was added later - so some hacks are required.

     3. The "override" callback will take care of all data sent to the
     user/front-end-HTTP-server,
        and so the first parameter to 'send_file' doesn't matter
        (just needs to be a scalar-ref, otherwise it will be mistaken
        for a file-name).

     4. The "$respond" parameter is a PSGI-related opaque object. Use
     it to generate the 'writer' object.

     5. The "$response" parameter is the original Dancer::Response
        object that Dancer generated.  Can be safely ignored, as we
		...
		callbacks => {
			override => sub {
				...
				my @http_headers = ( 'Content-Type' => 'text/plain',
						     ...
						     'Last-Modified' => strftime("%a, %d %b %Y %H:%M:%S GMT",
                                             gmtime),
